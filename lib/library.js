...
const STORAGE_PREFIX = 'iptv.library.v1';
// Pre-account-scoping key: the whole document used to live under one fixed
// key regardless of who was logged in, so any account used on the same
// browser read and wrote the same playlists (which hold Xtream credentials)
// as any other. Kept only as a one-time migration source - see
// runLegacyMigration, which claims and removes it for whichever account
// logs in first after this fix, so nobody else can inherit it by accident.
const UNSCOPED_LEGACY_KEY = 'iptv.library.v1';
...
// Set by components/SessionProvider as soon as the logged-in account is
// known. Every account gets its own key so a browser shared by several
// accounts (or several people) never mixes their playlists together.
...
// Called once per render from SessionProvider (see there for why not an
// effect) - idempotent when the id has not actually changed, so re-running
// it on every render costs nothing.
export function setLibraryUserId(userId) {
...
function getSnapshot() {
  ...
  // Also re-parse on a bare key change (switching accounts) even when the
  // new key's raw content happens to equal the previous cachedRaw (both
  // empty, most commonly) - comparing raw alone would keep serving the
  // previous account's cached document.
...
function write(doc) {
  ...
  const key = storageKey();
  // No account resolved yet - never fall back to writing the shared
  // unscoped key, that is exactly the bug this scoping exists to prevent.
  if (!key) return;
...
function runLegacyMigration() {
  // Waits for the account to be known instead of marking itself done - so
  // once setLibraryUserId resets migrationChecked for the newly-known user,
  // this actually runs for them rather than staying a no-op forever.
  if (migrationChecked || typeof window === 'undefined' || !currentUserId) return;
  ...
  // Claims the pre-account-scoping shared document, if one is still sitting
  // there, for whichever account logs in first after this fix - removing it
  // right after means no other account on this browser can inherit it too.
  try {
    const shared = storage.getItem(UNSCOPED_LEGACY_KEY);
    if (shared) {
      storage.removeItem(UNSCOPED_LEGACY_KEY);
      write(parseDoc(shared));
      return;
    }
  } catch {
    // Fall through to the older per-array migration below.
  }
