...
//
// The AI pick ("o que eu assisto hoje?") is the one feature every plan has,
// including the free one - what the plan buys is how often it may be used.
// Free gets one pick every AI_PICK_COOLDOWN_DAYS days; paid plans are
// unlimited (a cooldown of 0).
...
// A misconfigured value here would let a free account call the recommender in
// a loop, so it falls back to the default instead of to NaN/0.
const configuredPickCooldown = Number(process.env.NEXT_PUBLIC_AI_PICK_COOLDOWN_DAYS);
export const AI_PICK_COOLDOWN_DAYS =
  Number.isFinite(configuredPickCooldown) && configuredPickCooldown > 0
    ? Math.floor(configuredPickCooldown)
    : 15;
...
export const PLANS = {
  free: {
    ...
    sync: false,
    aiPicks: true,
    aiPickCooldownDays: AI_PICK_COOLDOWN_DAYS,
  },
  trial: {
    id: 'trial',
...
function build(planId, status, expiresAt, now, extra) {
  ...
  return {
    ...
    features: {
      ...
      sync: plan.sync,
      aiPicks: plan.aiPicks,
      // 0 means "as often as you like"; anything else is the wait between picks.
      aiPickCooldownDays: plan.aiPickCooldownDays,
    },
