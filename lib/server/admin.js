...
export async function dashboardStats() {
  const [totals, funnel, revenue, series] = await Promise.all([
    ...
    queryOne(`
      select
        count(*)::int as paying,
        -- Quem vai renovar sozinho - exclui quem ja cancelou (cancel_at_period_end)
        -- mas ainda tem acesso valido no periodo pago atual. "paying" conta os dois
        -- porque ambos tem acesso pago vigente; a receita recorrente so faz sentido
        -- pra quem realmente vai cobrar de novo.
        count(*) filter (where not cancel_at_period_end)::int as renewing
      from subscriptions
