...
// Inserts the user row plus its trial subscription, within an already-open
// transaction. Shared by password signup and OAuth signup - passwordHash is
// null for an OAuth-only account (see the users.password_hash migration).
async function createUserRow(run, { email, passwordHash, name }) {
...
export async function createUser({ email, password, name }) {
  ...
  return transaction(async (run) => {
    ...
    return createUserRow(run, { email, passwordHash, name });
  });
}

// Resolves a Google account into a user: an existing link wins, then an
// existing password account with the same e-mail gets linked (so someone who
// signed up by hand can start using "Continuar com Google" too), and only
// then does a brand new account get created. Callers must have already
// checked profile.emailVerified before reaching here.
export async function findOrCreateOAuthUser({ provider, providerAccountId, email, name }) {
  return transaction(async (run) => {
    ...
    const existing = await run(
      'select id, email, name, role, disabled_at from users where lower(email) = lower($1)',
      [email]
    );
    const user = existing.rows[0] || (await createUserRow(run, { email, passwordHash: null, name }));
