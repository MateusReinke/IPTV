...
// Undoes a scheduled cancellation (Stripe keeps the subscription active,
// with cancel_at_period_end true, until the paid period actually ends) - the
// customer portal already allows this buried in its own flow, but a direct
// button on /app/conta is faster than sending someone through the portal
// just to flip one setting back.
export async function resumeSubscription({ user }) {
  const row = await queryOne(
    'select provider_subscription_id from subscriptions where user_id = $1',
    [user.id]
  );
  const subscriptionId = row?.provider_subscription_id;
  if (!subscriptionId) throw new Error('Nenhuma assinatura para renovar.');

  const subscription = await stripeRequest(`/subscriptions/${subscriptionId}`, {
    cancel_at_period_end: false,
  });

  // The next customer.subscription.updated webhook would write the same
  // thing, but updating here too means the account page reflects it on the
  // very next reload instead of waiting on webhook delivery.
